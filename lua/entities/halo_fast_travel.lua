...
function ENT:UpdateDestinations( RemoveDestination )
    ...
    if not found then
        table.insert( Destinations, self )
    end
...
function ENT:PreInit()
    ...
    self:CallOnRemove( "UpdateDestinations", function( ent )
        ent:UpdateDestinations( true )
    end )
...
if CLIENT then
    ...
    function ENT:DrawDestinationsList(x, y, w, h)
        ...
        for index = start_index, start_index + math.floor(h / row_height) - 0 do
            ...
            if EnabledDestinations[index]:GetEnabled() == false then
                continue
            end
            ...
            draw.SimpleText(EnabledDestinations[index]:GetDestination(), "SP_QuanticoNormal", x + 15 + 20, y + ((index - start_index) * row_height) + (row_height / 2), Color(255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    
            -- Draw distance to Destination
            local distance = math.floor(EnabledDestinations[index]:GetPos():Distance(self:GetPos()) / 100) / 1
    ...
    function ENT:DrawScreen()
        ...
        for k, v in pairs( Destinations ) do
            ...
            if v.GetEnabled and v:GetEnabled() == true then
        ...
        table.sort( EnabledDestinations, function( a, b )
            return a:GetPos():Distance( self:GetPos() ) < b:GetPos():Distance( self:GetPos() )
        end )
...
if CLIENT then
    // Add a CLIENT hook to update the destinations list when the player is connected and spawned for the first time.
    hook.Add( "InitPostEntity", "HALOARMORY.FASTTRAVEL", function( ply )

        timer.Simple( 30, function()
            fast_travel_screens = ents.FindByClass( "halo_fast_travel" )
